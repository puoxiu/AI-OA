...
class OAUser(Base):
    ...
    leader_department = relationship("OADepartment", back_populates="leader", uselist=False, foreign_keys='OADepartment.leader_id')
    manager_departments = relationship("OADepartment", back_populates="manager", foreign_keys='OADepartment.manager_id')
    
    department_id = Column(Integer, ForeignKey('oa_department.id'), nullable=True)
    department = relationship("OADepartment", backref="staffs", foreign_keys=[department_id])
...
class OADepartment(Base):
    ...
    intro = Column(String(200), nullable=True)
     ...
     # 每个部门只能有一个leader
    leader = relationship("OAUser", back_populates="leader_department", uselist=False, foreign_keys='OADepartment.leader_id')
    leader_id = Column(String(22), ForeignKey('oa_user.uid'), nullable=True)

    # 部门的上级
    manager = relationship("OAUser", back_populates="manager_departments", foreign_keys='OADepartment.manager_id')
